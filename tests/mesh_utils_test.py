...
class MeshUtilsTest(unittest.TestCase):
  ...
  def test_add_domains(self):
    ...
    mesh1b.translate(Point(-1.1,1.1,0.0))
    mesh2 = UnitCubeMesh(1,1,1)
    mesh2.translate(Point(1.1,0.0,0.0))
    ...
    with self.assertRaises(ValueError):
      mt.add(mesh1, [5,6,7]) # use domain_id map of wrong type -> error
    with self.assertRaises(ValueError):
      mt.add(mesh1, {1:5,2:6,4:7}) # use domain_id map with missing domain -> error
    mt.add(mesh1, {1:5,2:6,3:7,4:8}) # use domain_id map (with additional domain)
